...
//     switch(status) {
//       case "present":
//         return { icon: CheckCircle, text: "Present", bg: "bg-green-500/20", textColor: "text-green-400" };
//       case "absent":
//         return { icon: XCircle, text: "Absent", bg: "bg-red-500/20", textColor: "text-red-400" };
//       case "late":
//         return { icon: Clock, text: "Late", bg: "bg-yellow-500/20", textColor: "text-yellow-400" };
//       default:
//         return { icon: AlertCircle, text: "Unknown", bg: "bg-gray-500/20", textColor: "text-gray-400" };
//     }
//   };
...
//               (stats.attendancePercentage || 0) >= 90 ? 'text-green-400' : 
//               (stats.attendancePercentage || 0) >= 75 ? 'text-yellow-400' : 'text-red-400'
//             }`}>
...
//                     <th className="px-5 py-3 text-left text-gray-300 text-sm font-semibold">Source</th>
//                   </tr>
//                 </thead>
...
//                             <span className="text-gray-400 text-sm">{record.source || "QR"}</span>
//                           </div>
//                         </td>
//                       </tr>
//                     );
...
export default function TeacherAttendancePage() {
  ...
  const getMoraleStatusColor = (score) => {
  ...
  const { records = [], stats = {}, chartData = [], aiInsights = {} } = attendanceData || {};
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {aiInsights && Object.keys(aiInsights).length > 0 && (
          <div className="mb-6 bg-gradient-to-r from-purple-900/30 to-indigo-900/30 rounded-xl p-5 border border-purple-500/30">
            <div className="flex items-center gap-2 mb-4">
              <Brain className="w-5 h-5 text-purple-400" />
              <h2 className="text-lg font-semibold text-white">AI Attendance Insights</h2>
              <Sparkles className="w-4 h-4 text-yellow-400" />
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              {/* Morale Score */}
              <div className={`rounded-xl p-3 border ${getMoraleStatusColor(aiInsights.moraleScore)}`}>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium">Morale Score</span>
                  <Award className="w-4 h-4" />
                </div>
                <p className="text-2xl font-bold mt-1">{aiInsights.moraleScore || 0}</p>
                <p className="text-xs mt-1">{aiInsights.moraleStatus || "Good"}</p>
              </div>
              ...
              <div className="bg-gray-800/50 rounded-xl p-3 border border-gray-700">
                ...
                <p className="text-sm font-medium text-white">{aiInsights.attendancePrediction || "Stable"}</p>
                <p className="text-xs text-gray-500 mt-1">Trend: {aiInsights.trend || "stable"}</p>
              </div>
              ...
              <div className="bg-gray-800/50 rounded-xl p-3 border border-gray-700">
                <div className="flex items-center gap-1 mb-1">
                  <AlertTriangle className={`w-4 h-4 ${
                    aiInsights.burnoutRisk === 'high' ? 'text-red-400' : 
                    aiInsights.burnoutRisk === 'medium' ? 'text-yellow-400' : 'text-green-400'
                  }`} />
                  <span className="text-xs font-medium text-gray-400">Burnout Risk</span>
                </div>
                <p className={`text-sm font-medium ${
                  aiInsights.burnoutRisk === 'high' ? 'text-red-400' : 
                  aiInsights.burnoutRisk === 'medium' ? 'text-yellow-400' : 'text-green-400'
                }`}>
                  {aiInsights.burnoutRisk === 'high' ? 'High' : 
                   aiInsights.burnoutRisk === 'medium' ? 'Medium' : 'Low'}
                </p>
                {aiInsights.burnoutReason && (
                  <p className="text-xs text-gray-500 mt-1 truncate">{aiInsights.burnoutReason}</p>
                )}
              </div>
              ...
              <div className="bg-gray-800/50 rounded-xl p-3 border border-gray-700">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-xs font-medium text-gray-400">Best Time</span>
                </div>
                <p className="text-sm font-medium text-white">{aiInsights.optimalTime || "7:30 AM"}</p>
                <p className="text-xs text-gray-500 mt-1 truncate">{aiInsights.optimalTimeReason}</p>
              </div>
            </div>
            ...
            {aiInsights.insights && aiInsights.insights.length > 0 && (
              <div className="bg-gray-800/30 rounded-lg p-3 mt-2">
                <p className="text-xs font-medium text-gray-400 mb-2">Personalized Insights</p>
                {aiInsights.insights.map((insight, idx) => (
                  <p key={idx} className="text-sm text-gray-300 mb-1">• {insight}</p>
                ))}
              </div>
            )}
          </div>
