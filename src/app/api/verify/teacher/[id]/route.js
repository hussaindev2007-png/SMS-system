...
// 👇 Helper function to get admin email from database
async function getAdminEmail() {
  try {
    // Find first active admin
    const admin = await Admin.findOne({ role: "admin", status: "active" }).select("email");
    if (admin && admin.email) {
      console.log(`📧 Found admin email in DB: ${admin.email}`);
      return admin.email;
    }
...
export async function GET(request, { params }) {
  try {
    ...
    console.log("🔍 Verifying teacher ID/code:", id);
    ...
    if (!existingAttendance) {
      try {
        ...
        // ✅ If teacher is late, send notification to ADMIN (from database)
        if (attendanceStatus === 'late') {
          const adminEmail = await getAdminEmail();
          if (adminEmail) {
            await addTeacherLateNotificationToQueue(adminEmail, teacher.name, currentTime);
            console.log(`📧 Teacher late notification queued for admin: ${adminEmail}`);
          } else {
            console.log(`❌ No admin email found, cannot send notification`);
          }
        }
